...
export default function RepoTree({ repoId }: { repoId: string }) {
  ...
  return (
    <div className="rounded-lg border border-gray-800 overflow-hidden">
      {/* Toolbar */}
      <div className="flex flex-wrap items-center gap-3 px-3 py-2 bg-gray-900/80 border-b border-gray-800">
        <div className="relative">
          <button
            onClick={() => setRefOpen((o) => !o)}
            className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md border border-gray-700 bg-gray-800 text-sm font-mono hover:border-gray-600"
          >
            <BranchIcon />
            <span className="max-w-[10rem] truncate">{ref || '…'}</span>
            <ChevronIcon />
          </button>
          {refOpen && tree && (
            <>
              ...
              />
              <div className="absolute left-0 top-full mt-1 z-20 w-64 max-h-72 overflow-y-auto rounded-lg border border-gray-700 bg-gray-900 shadow-xl">
                {tree.branches.length > 0 && (
                  <div>
                    <p className="px-3 py-1.5 text-[10px] uppercase tracking-wide text-gray-500 sticky top-0 bg-gray-900">
                      Branches
                    </p>
                    {tree.branches.map((b) => (
                      <button
                        key={b}
                        onClick={() => changeRef(b)}
                        className={`block w-full text-left px-3 py-1.5 text-sm font-mono hover:bg-gray-800 ${
                          b === ref ? 'text-blue-400' : 'text-gray-200'
                        }`}
                      >
                        {b}
                      </button>
                ...
                {tree.tags.length > 0 && (
                  <div>
                    <p className="px-3 py-1.5 text-[10px] uppercase tracking-wide text-gray-500 sticky top-0 bg-gray-900 border-t border-gray-800">
                      Tags
                    </p>
                    {tree.tags.slice(0, 50).map((t) => (
                      <button
                        ...
                        onClick={() => changeRef(t)}
                        className={`block w-full text-left px-3 py-1.5 text-sm font-mono hover:bg-gray-800 ${
                          t === ref ? 'text-blue-400' : 'text-gray-200'
                        }`}
                      >
                        {t}
                      </button>
        ...
        <nav className="flex items-center gap-1 text-sm min-w-0 flex-1 overflow-x-auto">
          <button
            ...
            className="text-blue-400 hover:underline font-mono shrink-0"
          >
            root
          </button>
          {crumbs.map((c) => (
      ...
      {tree?.commit && view.kind === 'tree' && (
        <div className="flex items-center gap-3 px-3 py-2 text-xs border-b border-gray-800 bg-gray-900/40">
          <span className="font-medium text-gray-300 truncate max-w-[12rem]">
            {tree.commit.author}
          </span>
          ...
          <tbody>
            {currentPath !== '' && (
              <tr
                ...
                onClick={goUp}
              >
                <td className="px-3 py-1.5 w-8 text-gray-500">
                  <FolderIcon open />
                </td>
                <td className="px-1 py-1.5 text-blue-400 font-mono" colSpan={2}>
                  ..
                </td>
              </tr>
            )}
              ...
              tree.entries.map((entry) => {
                ...
                return (
                  <tr
                    ...
                    className="border-b border-gray-800/40 hover:bg-gray-900/60 cursor-pointer group"
                    onClick={() =>
                      navigateTo(entryPath, isDir ? 'tree' : 'blob')
                    }
                  >
                    ...
                    <td className="px-1 py-1.5 font-mono text-gray-200 group-hover:text-blue-400 truncate max-w-0 w-full">
                      {entry.name}
                    </td>
                    <td className="px-3 py-1.5 text-right text-xs text-gray-600 font-mono whitespace-nowrap">
...
function FileViewer({
  ...
  const lines =
    !blob.binary && blob.encoding === 'utf-8'
      ? blob.content.split('\n')
      : [];

  return (
    <div>
      <div className="flex items-center justify-between gap-3 px-3 py-2 border-b border-gray-800 bg-gray-900/40">
        <div className="flex items-center gap-2 text-sm min-w-0">
          <FileIcon />
          <span className="font-mono text-gray-200 truncate">{blob.name}</span>
            ...
            <tbody>
              {lines.map((line, i) => (
                <tr key={i} className="hover:bg-gray-900/50">
                  <td className="px-3 py-0 text-right text-gray-600 select-none w-12 align-top border-r border-gray-800/60 sticky left-0 bg-gray-950">
                    {i + 1}
                  </td>
                  <td className="px-3 py-0 text-gray-300 whitespace-pre">
                    {line || ' '}
                  </td>
                </tr>
              ))}
